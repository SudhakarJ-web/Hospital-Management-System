...
interface GenericModuleTabProps {
  moduleName: string;
...
export default function GenericModuleTab({
  moduleName,
  records,
  headers,
  searchTerm,
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-b border-slate-100 pb-4">
        <div>
          <h2 className="text-base sm:text-lg font-black text-slate-900 tracking-tight uppercase">
            {moduleName} Control Ledger
          </h2>
          <p className="text-xs text-slate-500 mt-0.5 font-medium">
            Module Scope: <strong className="text-teal-700 font-bold">{moduleName}</strong> • Showing {filtered.length} Monitored Record(s)
          </p>
        </div>
          ...
          <thead className="bg-[#f8fafc] text-[10px] font-black uppercase text-slate-600 border-b border-slate-200 tracking-wider">
            <tr>
              {headers.map((h, idx) => (
                <th
                  key={idx}
                  className={`px-4 py-3.5 ${idx === headers.length - 1 ? "text-right" : ""}`}
                >
                  {h}
                </th>
              ))}
