...
interface MedicalOfficersManagementProps {
...
export default function MedicalOfficersManagement({
  records,
  searchTerm,
  onSearchChange,
  onOpenEdit,
  onDelete,
}: MedicalOfficersManagementProps) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 sm:p-6 space-y-4 sm:space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5 font-medium">
            Depot Officers & Pharmacist Licenses • Showing {filtered.length} Officer(s)
          </p>
        </div>
      ...
      <MetricsStrip
        ...
        pendingCount={filtered.filter((r) => r.status !== "Active").length}
        ...
        <table className="w-full text-left text-xs text-slate-700 min-w-[750px]">
          <thead className="bg-[#f8fafc] text-[10px] font-black uppercase text-slate-600 border-b border-slate-200 tracking-wider">
            <tr>
              ...
              <th className="px-4 py-3.5">License / Reg</th>
