...
export default function StockTab({
  ...
  const inStockCount = filtered.filter((r) => r.status === "Active" || r.status === "Completed").length;
  const lowStockCount = filtered.filter((r) => r.status === "Pending" || r.status === "Suspended").length;
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 sm:p-6 space-y-4 sm:space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 border-b border-slate-100 pb-4">
        <div>
          <h2 className="text-base sm:text-lg font-black text-slate-900 tracking-tight uppercase">
            Pharmacy & Drug Stock Ledger
          </h2>
          <p className="text-xs text-slate-500 mt-0.5 font-medium">
            SKU Batches, Reagents & Expiry Tracking • Showing {filtered.length} Monitored Item(s)
          </p>
        </div>
      ...
      <MetricsStrip
        ...
        totalLabel="Total Stock SKUs"
        activeLabel="Sufficient / Cleared"
        pendingLabel="Low Stock Alert"
      />
      ...
      <div className="border border-slate-200 rounded-xl overflow-x-auto shadow-xs">
        <table className="w-full text-left text-xs text-slate-700 min-w-[750px]">
          <thead className="bg-[#f8fafc] text-[10px] font-black uppercase text-slate-600 border-b border-slate-200 tracking-wider">
            <tr>
              <th className="px-4 py-3.5">SKU / Item ID</th>
              <th className="px-4 py-3.5">Medication / Reagent</th>
              <th className="px-4 py-3.5">Package Code</th>
              <th className="px-4 py-3.5">Batch Information</th>
              <th className="px-4 py-3.5">Available Stock & MRP</th>
